...
- `oc-work session do --repo <repository> --session <session> --prompt <prompt> --operative-branch <branch> [--base-branch <branch>] [--agent <name>]`
  - Executes a session action with a prompt.
  - Required: `--repo`, `--session`, `--prompt`, `--operative-branch`.
  - `--base-branch` is required when the operative branch does not yet exist.
  - `--agent` is optional.
